...
class CustomJsonNbtParser{
    ...
    private static function readValue(BinaryStream $stream) : Tag{
            ...
            if($inQuotes){ //anything is allowed inside quotes, except unescaped quotes
                if($c === '"'){
                    ...
                    if(!str_starts_with($value,"number(")) {
                        ...
                        }elseif(str_starts_with($value, "__ByteArray(")){
                            $retval = new ByteArrayTag(base64_decode(str_replace(["__ByteArray(", ""], "", $value)));
                        }elseif(str_starts_with($value, "__IntArray(")){
                            $retval = new IntArrayTag(explode(",", str_replace(["__IntArray(", ""], "", $value)));
                        }else{
        ...
        if($last !== "b" and $last !== "s" and $last !== "l" and $last !== "i" and $last !== "f" and $last !== "d"){
            $last = null;
        }
